...
{
	maxHp = static_cast<int>(obJectMaxHp);
	tempHp = static_cast<float>(maxHp);
	magnification = 1000 / maxHp;
}
...
void ObjectHp::draw(Vec2 pos, Vec2 offset)
{
	...
	DrawBoxAA(pos.x - 2, pos.y - 25, pos.x + maxHp * 10 + 2, pos.y - 13, 0x999999, true);
	DrawBoxAA(pos.x, pos.y - 23, pos.x + maxHp * 10, pos.y - 15, 0x000000, true);
...
{
	DrawBoxAA(pos.x, pos.y - 30, pos.x + maxHp * magnification, pos.y - 10, 0x000000, true);
	if (displayHp <= tempHp) {
		tempHp -= decreaseHp;
		decreaseHp += 0.03f;
	}
